...
public class Conta implements Cadastro {
    ...
    public void depositoDinheiro(final double valor) {
        Movimentaçao movimentacao = new movimentacao(this);
        movimentacao.setConfirmada(true);
        movimentacao.setTipo('C');
        movimentacao.setValor;
        saldo += valor;
        movimentaçoes.add(movimentacao);
    ...
    public void setNumero(String numero) {
        if (numero.matches("\\d{5}-\\d")) {
            throw new IllegalArgumentException("numero invalido. Deve estar no formato 99999-99");
